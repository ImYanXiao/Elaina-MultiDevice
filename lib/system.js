...
const SKIP_FILES = ['package.json', 'config.js']
...
function requestBufferWithProgress(url, label) {
  return new Promise((resolve, reject) => {
    https.get(url, res => {
      ...
      res.on('data', chunk => {
        ...
        if (total) {
          ...
          const barLength = 20
          const filled = Math.round((downloaded / total) * barLength)
          const bar = '█'.repeat(filled) + '-'.repeat(barLength - filled)
          process.stdout.write(`\r⬇️  ${label} [${bar}] ${percent}%`)
        }
      })
...
async function scanFolder(dirPath = '') {
  ...
  for (const item of list) {
    ...
    const remoteBuffer = await requestBufferWithProgress(item.download_url, item.path)
