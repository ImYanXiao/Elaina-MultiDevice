...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
        try {
            ...
            // Ensure the file is valid and not too small
            if ((res && res.status !== 200) || file.length <= 65536) {
                throw new Error('File is too small or invalid.');
            }

            // Determine the media type
            let mtype = this.getMimeType(type.mime, options);
     ...
     * @returns {String} - The media type ('image', 'video', 'audio', etc.)
     */
    getMimeType(mime, options) {
...
END:VCARD`.trim();
    ...
    async value(jid, path, quoted, options = {}) {
        try {
            ...
            // Determine the media type (image, video, audio, or document)
            let mediaType = this.getMediaType(mime, options);
