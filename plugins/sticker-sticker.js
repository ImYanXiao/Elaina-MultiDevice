...
let handler = async (m, { conn, args, usedPrefix, command }) => {
	...
	try {
		...
		if (/webp/g.test(mime)) {
			...
			stiker = await addExif(img, packname || stickpack, author || stickauth)
		} else if (/image/g.test(mime)) {
			let img = await q.download?.()
			stiker = await createSticker(img, false, packname || stickpack, author || stickauth)
		...
		console.log(e) // Tetap log error lengkap di console
		stiker = 'Terjadi kesalahan, coba lagi nanti!' // Pesan error singkat ke user
	} finally {
		m.reply(stiker)
	}
}
