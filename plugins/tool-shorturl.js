...
const sections = [
   {
	...
	rows: [
	    ...
	    {title: "Cutly", rowId: ".short " + args[0] + "cutly"},
...
//BITLY
if (args[1] == "bitly") {
	let bit = await (await fetch(`https://api-xcoders.site/api/tools/bitly?url=${args[0]}&apikey=xcoders`)).json()
	m.reply(pros).then(_=> conn.reply(m.chat, `${tesk}${bit.result}`,m))
}
//-------------

//CUTLY
if (args[1] == "cutly") {
	let cut = await (await fetch(`https://api-xcoders.site/api/tools/cuttly?url=${args[0]}&apikey=xcoders`)).json()
